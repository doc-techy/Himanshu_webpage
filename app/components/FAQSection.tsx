...
const FAQSection = () => {
  ...
  return (
    <section id="faq" className="relative w-full bg-white py-12 md:py-20 lg:py-24 overflow-hidden">
      ...
      <div className="relative mx-auto w-full max-w-7xl px-4 sm:px-6 lg:px-8 z-10">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-3 md:gap-4 max-w-5xl mx-auto items-start">
          {faqs.map((faq, index) => (
            <div
              key={index}
              className={`group rounded-xl bg-white border border-gray-200 shadow-sm hover:shadow-md hover:border-[#0b6f66]/30 transition-all duration-300 overflow-hidden ${
                openIndex === index ? "ring-2 ring-[#d5a028]/50" : ""
              }`}
            >
              <button
              ...
              >
                ...
                <div className={`flex-shrink-0 w-6 h-6 rounded-full flex items-center justify-center transition-all ${
                  openIndex === index 
                    ...
                    : "bg-[#0b6f66]/10 group-hover:bg-[#0b6f66]/20"
                }`}>
                  <svg
                    className={`w-3.5 h-3.5 transition-colors ${openIndex === index ? "text-white" : "text-[#0b6f66]"}`}
                    fill="none"
                    stroke="currentColor"
                    viewBox="0 0 24 24"
                  >
